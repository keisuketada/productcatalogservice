...
use hipstershop::product_catalog_service_server::{ProductCatalogService, ProductCatalogServiceServer};
use hipstershop::{Empty, Product, ListProductsResponse, GetProductRequest, SearchProductsRequest, SearchProductsResponse};

const FILE_NAME: &str = "./products.json";

async fn read_catalog_file() -> Vec<Product> {
    let content = fs::read_to_string(FILE_NAME).await.expect("Failed to load JSON");
...
impl ProductCatalogService for ProductCatalogServiceImpl {
    ...
    async fn get_product(
        ...
        Err(Status::new(tonic::Code::NotFound, format!("no product with ID {}", key_id)))
    }
